...
def pack_into_waves(tasks: list[Task], cap: int = 3) -> list[list[Task]]:
    ...
    force_serial = False  # next wave forced size 1 if previous wave hit a non-joiner
    while remaining:
        ...
        if not force_serial:
            while remaining and len(wave) < cap:
                ...
                if can_join_wave(next_task, wave):
                    ...
                    force_serial = True  # next wave starts with a non-joiner
                    break
            else:
                force_serial = False  # inner loop exhausted cap normally
        else:
            force_serial = False  # forced-serial wave consumed; reset
        waves.append(wave)
    return waves
```

**Note on `force_serial`:** This flag enforces the strict reading of "never reorders past a non-joiner" — a task that fails to join wave[k] becomes the seed of wave[k+1] and that wave is forced size 1, so a later disjoint task cannot leapfrog into the wave the non-joiner started. The earlier verbatim algorithm (without this flag) produced 2 waves for the test_contiguous_prefix_no_reorder case where the test asserts 3 waves. The implementation matches the test (and the spec's conservative-by-default doctrine).
